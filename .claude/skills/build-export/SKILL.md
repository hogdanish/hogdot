...
| `web-template_{release,debug}.{threads,nothreads}.wasm32.zip` | Not built since r14. CommonGrounds ships a GDExtension in one threaded preset, so nothing consumed these four. r1–r13 carry them. |
| `web-template_{release,debug}.threads.dlink.wasm32.zip` | The two shipped web templates: the builds above plus `dlink_enabled=yes`, threaded only since r14. From r15 the release one also takes `optimize=speed` (`-O3` over the web default `-Os`): the game's frame time outranks the few MB `-Os` saves; the debug one stays `-Os`. See "The dlink templates". The `.nothreads.dlink` pair is in r9–r13 only. |
