...
- **Status (2026-08-10, phase 10 tranche C):** the port is complete and the production initiative is through feature implementation — all 8 slices landed, `pre-commit run --all-files` passes, CommonGrounds boots on WebGPU, threaded web builds ship, and 4.7's new features are validated or implemented (HDR display output is live in Chrome). ⚠ **Treat every "renders correctly" claim older than 2026-08-10 as unproven.** That day's area-light gate found that **no positional light had ever cast a shadow on this backend** — a one-character-class scissor bug (RL-048, `fed7b78011`), silent, and invisible to every gate scene because none of them lit a shadow-casting positional light. Rendering has now been judged against a native reference for four gates; CommonGrounds has **not** been re-run since. No performance number exists. See `.claude/work/plans/STATUS.md` and `PORT-REPORT.md`.
...
`scons` · `ccache` 4.13.6 · `pre-commit` 4.6.1 · `emcc` **6.0.x** (⚠ moves under you — 6.0.5 → 6.0.6-git within one day; ask `emcc --version`, never a doc) · `glslang` 16.5.0 (⚠ no longer a build dependency — the precompile step died in `28a9960`; offline `webgpu_tests/shader_corpus/` tooling only). All installed and verified 2026-08-06.
