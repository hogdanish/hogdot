...
- **Performance (2026-09-01):** the browser perf bed is `webgpu_tests/perf/` (CommonGrounds-shaped scenes, rAF busy time, WebGPU API census, screenshot A/B). Two frame-dominating defects shared with GodotWebGPU 4.6.2 were fixed (whole-slice buffer flushes, one queue submit per render pass): `world` 15.0 → 1.5 ms CPU/frame. The shader baker silently reused unbaked containers across exports; fixed. Ledger for CommonGrounds: `webgpu_tests/perf/HANDOFF.md`.
